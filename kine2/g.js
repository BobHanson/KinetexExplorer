...
function jsgraph(svar,sflags,j0){
	...
	Info.title="<center><h3>"+stitle+"</h3><b>"+stitle1+"</b><br><a href=javascript:print()>Print</a>&nbsp;&nbsp;<a href=javascript:close()>Close</a></center>"
...
{
 if(!havedata)parseinfo()
 if(n__==0){dogoto("#conc");return alert("No data to plot! Click on 'Continue'")}
 var s=""
...
function xyof(i,j,isy,iround){
 if(isy){
  ...
  var x=(isvsc?T[i][j]*(vscn==2?T[i][j]:1):nT[j])
