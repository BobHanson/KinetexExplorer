//6:12 AM 2/13/2004 + 9:29 PM 4/26/2005
/*
...
function getStyle(){
 ...
 s+="a.pageref{color:darkred}"
 s+="a.concept{color:darkgreen;text-decoration:none}"
 s+="a.codelink{color:#C0C0C0;text-decoration:none}"
 ...
 */
 ...
 if(!CI[thisPage]){
	...
	npages=CI[thisPage].length/2
	s=ichap+"/"+thisPage+(iframe && npages>1?"."+iframe:"")
	sout="<b>"+symclean(title)+" <a class=codelink target=_top href="+parent.location.href.split("?")[0].replace(/\/index.htm/,(parent.location.href.toUpperCase().indexOf("FILE/")>=0?"":"/index.htm"))+"?"+s+">"+s+"</a>"+"</b><p>"+getThePage(iframe,true,iscript,ScriptList.length)+"<pre>\n\n\n\n\n\n\n\n</pre>"
 }	
 dowrite(parent.fraConcept,sout)
}
...
function getThePage(n,iwrap,nwrap,ntotal){
 ...
 s="<img border=0 src="+s.substring(0,s.lastIndexOf("/"))+"/"+(sgif.indexOf("..")<0?"img/":"")+sgif+">"
