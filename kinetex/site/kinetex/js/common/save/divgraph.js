...
function GRdrawgraph(f_of_x,Data,Labels,Info,doc){
	...
	if(f_of_x.length){
		if(getxrange){
			if(isNaN(GR.xmin))GR.xmin=parseFloat(GRprompt("X minimum?",-10))
			if(isNaN(GR.xmax))GR.xmax=parseFloat(GRprompt("X maximum?",10))
		}
	...
	if(!GR.append){
		...
		if(title.length){
			if(GR.anchorname.length==0 && GR.winwidth>0)title="<table><tr><td width="+GR.winwidth+"px>"+title+"</table>"
			GRnewlabel(title,0,0,"black class=title")
		}
	...
	if(Data.length && !labelsonly && !isNaN(Data[0][0])){
		...
		if(dotrendline){
			...
			if(dotrendeqn && !isNaN(trendx)&&!isNaN(trendy)){
				s="<b>"+YvsX[0]+" = "+GRroundoff(GR.L.m,trenddig)+"("+YvsX[1]+") + "+GRroundoff(GR.L.b,trenddig)+"<br>r<sup>2</sup> = "+GRroundoff(GR.L.r2,2)+"</b>"
...
function GRcleargraph(setdefaults){
	...
	GR.defstyle=".bodyfont {font-size:10pt;color:black}\n"+
