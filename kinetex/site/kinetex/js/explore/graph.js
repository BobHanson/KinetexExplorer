...
function Graph_setXmax(x,smodel){
	if(arguments.length<2)smodel=thismodel 
	GraphInfo[smodel].xmaxfromuser=x
}
...
function Graph_initialize(smodel){
	...
	if(G.graphwindow && G.graphwindow.closed)G.isinitialized=false
 	if(G.isinitialized){
		if(G.graphwindow)G.graphwindow.focus()
		return true
	}
	Graph_setup(smodel)
	G.isinitialized=true
...
function Graph_setup(smodel){
 Expt_reset(smodel)
 var G=GraphInfo[smodel]
 G.xmax=(G.xmaxfromuser?G.xmaxfromuser:ExptInfo[smodel].endtime)
 if(ihaveanalysisjs){
...
//	if(autostart)alert("You can't use Model_init(1) on a page that automatically pops up a new window. Netscape considers this a security risk.")
	return
 }
