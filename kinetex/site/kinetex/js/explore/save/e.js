...
function Expt_loop(smodel){
	...
	if(iserror
		...
		if(ihaveanalysisjs && MouseInfo.event && MouseInfo.event=="up"
			&& AnalysisInfo[smodel].haveinterface && AnalysisInfo.thisamodel==smodel){
...
function Expt_setDefaults(smodel){
	...
	ExptInfo[smodel].haveinterface=false
...
function Expt_show(smodel,imustdo){
	...
	s+="\n</tr></table></form>"
	document.write(s)
	ExptInfo[smodel].haveinterface=true
}
...
function Expt_start(smodel,issetuponly){
	...
	if(ihavegraphjs)Graph_reset(smodel)
...
function Expt_updateInterface(smodel){
	if(!ExptInfo[smodel].haveinterface)return
