//CHECKJS  D:\js\kinetex\model.js 1/31/2003 6:28:30 AM
//model.js
thismodel=""
MINTEMP=10
TEMPFACTOR=Math.E
...
function Model_changeTemp(T,smodel){ //this one must adjust k values
	...
	for(var i in ModelInfo[smodel].Constant){
		if(i.charAt(0)=="k"){
			...
			if(ModelInfo[smodel].haveinterface){
...
function Model_createNew(smodel){
	...
	ModelInfo[smodel].haveinterface=false
...
function Model_refresh(isfromform,smodel){
	if(arguments.length<2)smodel=thismodel
	if(!ModelInfo[smodel].haveinterface)return
	A=new Array()
...
function Model_show(smodel,imustdo){
	...
	s+="</form>"
	document.write(s)
	ModelInfo[smodel].haveinterface=true
}
