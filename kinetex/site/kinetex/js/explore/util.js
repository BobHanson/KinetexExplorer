...
//12:13 PM 7/13/2004 Opera "feature" parseFloat("")=0, not NaN

debugdiv=false && true
divlist=""
MINUS="&minus;" //by adding symbols.js AFTER util.js, this can be browser-dependent
...
function Util_createDiv(id,s){
	...
	var sout="\n<style>#"+id+"{position:absolute;left:0;top:600}</style>"
	sout+="\n<div id="+id+">"+s+"\n</div>"
...
function Util_fixEqn(e,smodel){
	...
	while(i<e.length){
		...
		if(e.charAt(i)=="]" && !isNaN(parseFloat(e.substring(i+1,e.length)+"x"))){
			e=e.substring(0,i+1)+"^"+e.substring(i+1,e.length)
		}
		if(e.charAt(i)=="^"){
			n=parseFloat(e.substring(i+1,e.length)+"x")
			sn=""+n
...
function Util_fixExponents(s){
	if(s.indexOf("<sub>")>=0||s.indexOf("<sup>")>=0)return s
	var i=0
	var n=0
	var sn=n
	var c=""
...
// k2[A][B]2
	...
	sout=sout.replace(/\_\_\_/g,"<sup>3"+MINUS+"</sup>")
	sout=sout.replace(/\_\_/g,"<sup>2"+MINUS+"</sup>")
	sout=sout.replace(/k\_/g,"k<sub>"+MINUS+"</sub>")
	sout=sout.replace(/\_/g,"<sup>"+MINUS+"</sup>")
	return sout
}
