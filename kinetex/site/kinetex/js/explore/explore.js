//explore.js 12:27 PM 7/11/2004 requires simulate.js (but not vice-versa)

function Explore_getHeaders(what,isqonly){
	if(!SimInfo.Exp[what])return ""
	var main=SimInfo.Exp[what][0]
	var sub1=SimInfo.Exp[what][1]
	return Explore_header(main,sub1,isqonly)
}

function Explore_header(main,sub1,isqonly){
	var s="<h3>"+(isqonly||main==""||main.indexOf("Quiz:")>=0?"":"Exploration: ")+main+"</h3><p>"
	if(!isqonly && sub1)s+=sub1+"<p>"
	return s
}

function Explore_sayh(what,isqonly){
	if(arguments.length<2)isqonly=false
	SimInfo.thisexp=what
	document.write(Explore_getHeaders(what,isqonly))
}

function Explore_say(main,sub1,isqonly){
	if(arguments.length<3)isqonly=false
	if(arguments.length<2)sub1=""
	document.write(Explore_header(main,sub1,isqonly))
}

function Explore_sayq(what){
	if(arguments.length==0)what=SimInfo.thisexp
	document.write(Explore_getQuestions(what,-1))
}

function Explore_getQuestions(what,i0){
	var s=""
	if(!SimInfo.Exp[what])return ""
...
function Explore_getMainHeader(what){
	if(!SimInfo.Exp[what])return ""
...
SimInfo.Exp["oscil"]=new Array("Heartbeat in a Flask (first 20 seconds)",""
...
SimInfo.Exp["oscil2"]=new Array("Heartbeat in a Flask (200 seconds)",""
,"<b>Oscillating reactions</b> involve positive feedback to create a situation where the concentration of one or more reactants, intermediates, or products fluctuates with time."
,"In oscillating reactions, the relationship between reaction rate and concentration breaks down entirely.<p><img src=$CDIR/rate1x.gif>"
)
